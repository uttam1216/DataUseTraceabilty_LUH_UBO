...
def _get_hash(data):
    ...
    # commented on 23rd Sept by Uttam
    '''
    ...
    import json
    data = json.dumps(data, sort_keys=True)  # convert into string
    hash_data = str(hash(data)).encode()  # get hash of the string with gives an integer, then cast into string
    # and then encode into byte

    return hash_data
    '''
    # data of type tuple
    """    #the tuple could not be managed because each time the hash changes upon run with tuple data
